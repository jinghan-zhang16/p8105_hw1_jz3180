...
**The data in the penguins dataset is stored in a tibble, with `r nrow(penguins)` observations (rows) and `r ncol(penguins)` categories/variables (columns) for each observation. These variables specifically studied are `r colnames(penguins)`. The mean flipper length is `r mean(pull(penguins, flipper_length_mm), na.rm = TRUE)` mm.**
